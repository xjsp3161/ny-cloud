...
public class SysUserGroupPkController {
    ...
    public HttpResponse userNoRelationRoleList(UserGroupDto dto) {
    ...
    public HttpResponse userRoleList(UserGroupDto dto) {
        return HttpResponse.resultSuccess(sysUserGroupPkService.loadGroupUsers(dto));
    }

    @ApiOperation(value = "用户组关联用户批量保存",  notes = "保存多个SysUserRolePk对象")
    @PostMapping("/batchSave")
    public HttpResponse batchSave(@RequestBody List<SysUserGroupPk> list) {
